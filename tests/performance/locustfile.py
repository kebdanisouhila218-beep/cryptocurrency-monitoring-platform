...
"""
Tests de performance avec Locust pour CryptoTracker API
...
class CryptoTrackerUser(HttpUser):
    ...
    def on_start(self):
        """Exécuté au démarrage de chaque utilisateur - Login"""
        # Créer un utilisateur de test ou se connecter
        self.token = None
        # Utiliser un identifiant unique basé sur l'ID du Locust user
        self.username = f"loadtest_user_{self.environment.user_count}_{int(time.time() * 1000)}"
        ...
        # Se connecter avec l'utilisateur créé (form-data pour OAuth2)
        ...
        if login_response.status_code != 200:
            ...
            print(f"Headers: {login_response.headers}")
        else:
        ...
        if login_response.status_code == 200:
            ...
            print(f"✅ Login réussi: {self.username}")
        else:
            ...
            if login_response.status_code == 200:
                ...
                self.headers = {}
                print(f"❌ Login admin échoué")

    # ===== TESTS API PUBLIQUES =====
    
    @task(10)
    def health_check(self):
        """Test endpoint health - Haute fréquence"""
    ...
    def get_public_stats(self):
        ...
        self.client.get("/public/stats")

    # ===== TESTS API PROTÉGÉES =====
    
    @task(20)
    def get_prices(self):
        """Test récupération des prix - Très haute fréquence"""
        if self.token:
            self.client.get("/prices", headers=self.headers, catch_response=True)
        else:
            # Tenter sans authentification (pour voir les erreurs 401)
            self.client.get("/prices", catch_response=True)
...
def on_test_start(environment, **kwargs):
    ...
    print("=" * 60)
    print("🚀 Démarrage des tests de performance CryptoTracker")
    print("=" * 60)
